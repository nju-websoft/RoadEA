...
class KG:
    """
    Class for single KG. All its entities, relations, attributes and values remain original texts.
    """
    def __init__(self, relation_triples, attribute_triples):
        ...
        self.values_set, self.values_list = None, None
        self.entities_num, self.relations_num, self.attributes_num, self.values_num = None, None, None, None
        ...
        self.ent_attr_value_dict = None
        self.attr_value_dict = None
        self.attr_value_list = None
        self.value_attr_concate = None
    ...
    def set_attributes(self, attribute_triples):
        ...
        self.values_list = list(self.values_set)
        self.values_num = len(self.values_set)
    ...
    def parse_attributes(self):
        ...
        self.ent_attr_value_dict = dict()
        self.attr_value_dict = dict()
        self.attr_value_list = list()
        self.value_attr_concate = dict()
        for ent, attr, val in self.local_attribute_triples_set:
            attrs = self.entity_attributes_dict.get(ent, set())
            attrs.add(attr)
            self.entity_attributes_dict[ent] = attrs
            # **********{ent, [value]} dictionary************
            ent_attr = self.ent_attr_value_dict.get(ent, set())
            ent_attr.add(val)
            self.ent_attr_value_dict[ent] = ent_attr
            # **********{attr, [value]} dictionary***********
            attr_value = self.attr_value_dict.get(attr, set())
            attr_value.add(val)
            self.attr_value_dict[attr] = attr_value
            # ***********{value, attr} dictionary************
            self.value_attr_concate[val] = attr
        for item in self.attr_value_dict.keys():
            self.attr_value_list.append(list(self.attr_value_dict[item]))

        print("entity attributes dict:", len(self.entity_attributes_dict))

    def set_id_dict(self, entities_id_dict, relations_id_dict, attributes_id_dict, values_id_dict):
        self.entities_id_dict = entities_id_dict
        self.relations_id_dict = relations_id_dict
        self.attributes_id_dict = attributes_id_dict
        self.values_id_dict = values_id_dict
